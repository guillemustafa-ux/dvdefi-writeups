...
contract UnstoppableChallenge is Test {
    ...
    function test_unstoppable() public checkSolvedByPlayer {
        // Break the vault's 1:1 shares<->assets invariant by donating tokens
        // directly (no deposit => no shares minted). From then on every
        // flashLoan reverts on `convertToShares(totalSupply) != balanceBefore`,
        // so the monitor's try/catch pauses the vault and hands ownership back.
